...
\begin{frame}[fragile]{Использование имён членов вне области видимости класса}
  \begin{itemize}
    \item область видимости наследников \vspace{1em}
...
\begin{frame}{Нестатические поля и методы}
  Поля:
  \begin{itemize}
    \item являются подобъектами объекта класса \vspace{0.5em}
    \item тип должен быть полностью определён в точке объявления \vspace{0.5em}
    \item не могут быть \lstinline{auto}, \lstinline{extern}, \lstinline{thread_local} \vspace{0.5em}
    \item размер $\geq 1$ \vspace{0.5em}
...
\begin{frame}{Примеры нестатических полей и методов}
  \begin{lstlisting}[basicstyle=\ttfamily\srcbigsize]
    struct S
    {
      const int a = 10;
